...
const { collectionDelete } = useGlobalOpeners()
const toast = useToast()

const popupData = computed(() => collectionDelete.data.value)
const isSubmitting = ref(false)

const _delete = useMutation({
  mutationFn: $trpc.collectionRouter.delete.mutate,
  onError: () => toast.add({ title: 'We could not delete the collecion' }),
  onSuccess: (collection) => {
    queryClient.invalidateQueries({ queryKey: ['collections', popupData.value?.workspaceId, popupData.value?.projectId] })
    isSubmitting.value = false
    toast.add({
      title: `${collection.name} was deleted`,
...
const submit = () => {
  ...
  if (popupData.value) {
    _delete.mutate(popupData.value)
  }
}
...
const deleteFormSchema = computed(() => {
  return z.object({
    name: z.string().refine(value => value === popupData.value?.name, {
      message: 'Please enter the name of the collection.'
    })
  })
})
